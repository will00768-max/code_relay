...
PROXY_API_KEY = os.getenv("PROXY_API_KEY", "").strip()  # 留空则不校验
...
def responses_input_to_messages(input_data) -> list[dict]:
    ...
    for item in input_data:
        ...
        # assistant 消息特殊处理：从 content blocks 中分离 reasoning 和文本
        if role == "assistant" and isinstance(raw_content, list):
            ...
            for block in raw_content:
                ...
                if btype in ("text", "output_text"):
                    ...
                    # Codex Responses API: reasoning block 的文本在 summary 数组或 text 字段
                    summary = block.get("summary") or block.get("encrypted_content")
...
def chat_completion_to_response(completion, model: str) -> dict:
    ...
    # 构建 content blocks：若有 reasoning_content，先插入 reasoning block
    # Codex 下一轮会把这个 block 原样回传，我们再从中提取 reasoning_content
...
async def stream_response_events(
    ...
    full_text = ""
    full_reasoning = ""          # 收集 DeepSeek thinking 模式的 reasoning_content
    ...
    async for chunk in stream:
        ...
        # ────── reasoning_content delta（DeepSeek thinking 模式） ──────
    ...
    if msg_item_announced:
        ...
        # 构建 content blocks，带 reasoning 供 Codex 下轮回传
        done_content_blocks: list[dict] = []
        if full_reasoning:
            done_content_blocks.append({
                "type": "reasoning",
                "text": full_reasoning,
                "summary": [{"type": "summary_text", "text": full_reasoning}],
            })
        done_content_blocks.append({"type": "output_text", "text": full_text, "annotations": []})
    ...
    final_output = []
    if msg_item_announced:
        final_content_blocks: list[dict] = []
        if full_reasoning:
        ...
        final_content_blocks.append({"type": "output_text", "text": full_text, "annotations": []})
        final_output.append({
